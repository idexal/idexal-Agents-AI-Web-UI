...
describe('DocumentTitle', () => {
  ...
  it('uses the generic title when the build provides no title', () => {
    ...
    const mounted = render(<DocumentTitle title="First title" productTitle="Idexal Agents" />)
    expect(document.title).toBe('First title — Idexal Agents')
    mounted.unmount()
    expect(document.title).toBe('Idexal Agents')
  })
})
